...
export class ApiKeyStore {
  ...
  public static addKey(type: string, value: string, note: string): void {
    ...
    MainLogger.info(`[Security Audit] Thêm mới API key an toàn cho provider: ${type}`, {
      correlationId: 'SECURITY_AUDIT_KEY_ADD'
    });
  }
  ...
  public static deleteKey(id: string): void {
    ...
    if (target) {
      MainLogger.info(`[Security Audit] Xóa API key của provider: ${target.type}`, {
        correlationId: 'SECURITY_AUDIT_KEY_DELETE'
      });
    }
  }
  ...
  public static getKeysForUI(): any[] {
    ...
    const envTypes = [
      'GEMINI_API_KEY', 'FAL_KEY', 'OPENAI_API_KEY',
      'ELEVENLABS_API_KEY', 'OMNI_VOICE_API_KEY',
      'KLING_API_KEY', 'RUNWAY_API_KEY', 'PIKA_API_KEY',
      'LUMA_API_KEY', 'PIXVERSE_API_KEY', 'HAILUO_API_KEY', 'WAN_API_KEY'
    ];
  ...
  public static getRawKeys(type: string): string[] {
    ...
    // Fallback sang process.env nếu không có key nào trong store
    if (rawKeys.length === 0 && process.env[type]) {
      const raw = process.env[type] || '';
      const envKeys = raw.split(/[,;]/).map(k => k.trim()).filter(k => k.length > 0);
      return envKeys;
    }
  ...
  public static getKey(type: string): string {
    const keys = this.getRawKeys(type);
