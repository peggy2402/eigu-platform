...
- **API (`feedback.service.ts`):** Tạo service và Prisma model `Feedback` (userId, content, imageUrl). Tích hợp gửi Discord Webhook. Validation chống spam (tối đa 3 lần/ngày). Sửa lỗi `Property 'feedback' does not exist on type 'PrismaService'`.
