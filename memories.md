...
3. **Phát triển Web Dashboard (Next.js - Cổng 3000):**
   - Xây dựng giao diện UI Dark Mode hiện đại sử dụng React Flow để biểu diễn trực quan luồng công việc (Video -> FFmpeg -> Browser -> TikTok).
   - Tích hợp Socket.io-client để thanh Progress Bar nhảy tự động theo thời gian thực (Real-time).

4. **Xây dựng Desktop Worker (Electron + Node.js):**
   - Khởi tạo script chạy ngầm.
   - Bổ sung giao diện GUI nhỏ bằng Electron (index.html) để dễ theo dõi.
