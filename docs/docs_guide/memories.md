...
## Phase 20: Phiên làm việc 26/07/2026 (23:00 GMT+7) — Realtime File Watcher, Multi-tenant Data Isolation, Refactor Kiến Trúc SaaS Credit (No BYOK) & AI Director Logging

### 20.1 Realtime File Watcher Tệp Dự Án (.eigu) & Xử Lý Tệp Active Bị Xóa
- **Thời gian xử lý:** 26/07/2026 12:00 - 12:40 GMT+7
- **Bối cảnh & Yêu cầu:** Người dùng xóa tệp `.eigu` trực tiếp từ Finder/File Explorer trong lúc ứng dụng đang chạy. UI phải lập tức cập nhật trạng thái missing (gạch ngang, đỏ, "Không tìm thấy file") trong Recent Projects list và xử lý an toàn cho phiên làm việc active.
- **Giải pháp Triển khai:**
  - Sử dụng `fs.watch` thuần của Node.js để theo dõi từng đường dẫn file `.eigu` trong danh sách Recent Projects và tệp dự án active (`currentFile.filePath`).
  - Khi phát hiện tệp dự án active bị xóa: Tự động reset toàn bộ state trong `StudioStore` (project, scenes, characters, assets, filePath) về rỗng, đóng phiên làm việc và chuyển hướng UI về màn hình Welcome/Recent Projects kèm thông báo Toast cảnh báo.
  - Cập nhật đánh dấu missing realtime trên danh sách Recent Projects khi tệp bị xóa trên đĩa.

### 20.2 Cấu Trúc Lưu Media Theo Chuẩn Adobe (Output Đính Kèm Cùng Dự Án)
- **Thời gian xử lý:** 26/07/2026 12:50 - 13:10 GMT+7
- **Vấn đề:** `AIVideoPipeline` tạo thư mục ngẫu nhiên dạng `Downloads/eigu/ai_outputs/<Date.now()>/` hoàn toàn không liên quan tới dự án `.eigu` đang mở.
- **Giải pháp Triển khai:**
  - Tối ưu cấu trúc lưu media theo chuẩn Adobe: Media cache và render được lưu cùng vị trí với tệp dự án tại `[Directory]/[ProjectName]_media/` (Ví dụ: `tesst_media/`).
  - **Khắc phục lỗi crash khởi động:** Sửa lỗi eager initialization của `new AIVideoPipeline()` trong constructor của `SceneService` vốn làm app crash ngay lúc boot khi chưa có project nào được mở.

### 20.3 Thu Hẹp Phạm Vi Dự Án & Cập Nhật 32 File Specification Markdown
- **Thời gian xử lý:** 26/07/2026 13:40 GMT+7
- **Điều chỉnh Phạm vi:** Tập trung toàn lực vào **AI Video Studio Sản xuất Hàng loạt (Batch Production 100+ Video)**, loại bỏ hoàn toàn tính năng Auto-Publish lên mạng xã hội.
- **Cập nhật Tài liệu:** Đồng bộ chỉnh sửa 32 file markdown spec trong `docs/modules/ai-video-studio/` phản ánh đúng phạm vi sản xuất hàng loạt.

### 20.4 Khắc Phục Lỗi Bảo Mật Rò Rỉ Dữ Liệu Đa Người Dùng (Multi-tenant Data Isolation)
- **Thời gian xử lý:** 26/07/2026 14:20 - 14:50 GMT+7
- **Lỗi Bảo Mật Nghiêm Trọng:** User 2 đăng nhập bằng tài khoản khác hoàn toàn lại nhìn thấy được dự án AI của User 1 do backend trả về danh sách không lọc đúng chủ sở hữu.
- **Giải pháp Triển khai:**
  - Áp dụng nguyên tắc **Strict Isolation**: Mọi API endpoints (`GET /api/ai-video/projects`, `GET /api/chat/history`, `/api/feedback`) bắt buộc đọc `req.user.id` / `req.user.email` từ JWT Token và lọc dữ liệu bằng `where: { userId }`.
  - Cấm 403 tuyệt đối với tài khoản không phải Admin đối với các route quản trị (`/api/users`, `/api/security/*`).
  - Viết và chạy thành công script test E2E `test-user-data-isolation.js` (PASS 100%) và UI Electron script `test-manual-ui-isolation.js` (PASS 100%).

### 20.5 AI Director Logging & Error Propagation Chi Tiết Trên UI
- **Thời gian xử lý:** 26/07/2026 15:00 - 15:15 GMT+7
- **Vấn đề:** Khi bấm "AI Tạo" (sinh kịch bản), Toast hiển thị thông báo chung chung *"Không thể khởi tạo kịch bản từ AI Director (IPC/Engine response empty)"* và file `eigu.log` không có log chi tiết.
- **Giải pháp Triển khai:**
  - Bổ sung `MainLogger.info` (log payload, mode, số lượng char, sự hiện diện của API key Gemini/OpenAI không lộ secret) và `MainLogger.error` trong IPC `ai-video-generate-prompts` ([studio.ipc.ts](file:///Users/peggy2402/Projects/eigu-platform/apps/desktop/src/main/ipc/handlers/studio.ipc.ts#L710)).
  - Sửa `showInputDialog()` ([studio-main.js](file:///Users/peggy2402/Projects/eigu-platform/apps/desktop/src/assets/js/ui/studio-main.js#L1402)) và [ai-video-ui.js](file:///Users/peggy2402/Projects/eigu-platform/apps/desktop/src/assets/js/ui/ai-video-ui.js#L110) trích xuất chính xác thông điệp lỗi gốc `error.error` để hiển thị trên Toast.
  - Thêm bảng **Trạng Thái & Cài Đặt AI Provider (AI Engine Matrix)** trực quan trong giao diện Cài đặt ([settings.js](file:///Users/peggy2402/Projects/eigu-platform/apps/desktop/src/assets/js/ui/settings.js#L150)).

### 20.6 Refactor Kiến Trúc SaaS Credit & Quản Lý Key Tập Trung (No BYOK)
- **Thời gian xử lý:** 26/07/2026 22:45 - 23:00 GMT+7
- **Chuyển Đổi Kiến Trúc SaaS Top-Level:**
  1. **Loại bỏ gọi Provider trực tiếp từ Client Desktop:** `AIVideoPipeline` ([ai-video-pipeline.service.ts](file:///Users/peggy2402/Projects/eigu-platform/apps/desktop/src/main/services/ai-video-pipeline.service.ts)) không đọc/ghi `api_keys_secure.json` local để tự gọi API bên ngoài nữa. Mọi lệnh **Sinh kịch bản (Generate Prompts)** và **Render Video** bắt buộc gửi request qua Backend API (`apps/api`) đi kèm JWT Token đăng nhập.
  2. **Quản lý Key Tập Trung Phía Server (No BYOK):** Toàn bộ API Key (Gemini, OpenAI, Fal.ai, Veo, Runway, Kling...) do Nền tảng sở hữu và quản lý tập trung trên Server Backend. Màn hình "Bể chứa API Keys" trên Desktop App được khóa/ẩn 100% đối với tài khoản không phải Admin.
  3. **Billing Trừ Credit Theo Lượt Dùng (UserCredit & CreditTransaction):**
     - Thêm cột `creditBalance` vào model `User` và tạo model `CreditTransaction` trong [schema.prisma](file:///Users/peggy2402/Projects/eigu-platform/apps/api/prisma/schema.prisma#L37).
     - **API Endpoints mới**: `POST /api/ai-video/generate-prompts`, `POST /api/ai-video/render`, `GET /api/ai-video/user-credit`, `GET /api/ai-video/jobs/:jobId/status` trong [ai-video.controller.ts](file:///Users/peggy2402/Projects/eigu-platform/apps/api/src/ai-video/ai-video.controller.ts#L250).
     - **Pre-render Credit Check**: Backend kiểm tra `user.creditBalance >= provider.creditCost` trước khi enqueue job. Nếu không đủ, trả lỗi `INSUFFICIENT_CREDIT`.
     - **Post-render Deduction**: Hàm `deductCreditOnSuccess` trong [ai-video.service.ts](file:///Users/peggy2402/Projects/eigu-platform/apps/api/src/ai-video/ai-video.service.ts#L880) thực hiện trừ credit và lưu vết `CreditTransaction` **chỉ khi job render hoàn tất thành công**.
  4. **Dual-Layer Guardrail**: `CostGuardrailService` ([cost-guardrail.service.ts](file:///Users/peggy2402/Projects/eigu-platform/apps/desktop/src/main/services/cost-guardrail.service.ts#L89)) kiểm tra 2 lớp bảo vệ (Số dư Credit trên Server + Ngân sách dự án).
  5. **Cập nhật Spec**: `03-Architecture.md`, `08-Provider-System.md`, `29-Security.md`, `20-Cost-Management.md`.
  6. **Automated Verification**: Chạy script E2E `test-saas-credit-architecture.js` xác nhận PASS 100% tất cả luồng đăng ký, sinh kịch bản qua server key, và submit render job.
