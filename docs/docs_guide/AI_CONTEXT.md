   ...
   - **Production & Client Readiness First Mindset Skill (CRITICAL PRODUCTION DEPLOYMENT SKILL):**
    ...
    - **Mandatory UI Text Internationalization with `data-i18n` (CRITICAL I18N SKILL):**
     ...
     - **Modern Dark Custom Form Controls & Select Dropdowns (CRITICAL UI/UX SKILL):**
       ...
       3. **CHUYỂN ĐỔI THEME TỰ ĐỘNG KHÔNG HARDCODE MÀU NỀN TỐI HỘP CỨNG:** TUYỆT ĐỐI KHÔNG hardcode màu nền tối cố định kiểu `background: rgba(15, 23, 42, 0.8)` hoặc `#0f172a` cho các container/card trong HTML/JS inline. BẮT BUỘC sử dụng biến CSS (`var(--bg-card)`, `var(--bg-primary)`, `var(--text-primary)`, `var(--border-color)`) để khi người dùng chuyển giữa **Giao diện Tối (Dark)** và **Giao diện Sáng (Light)**, toàn bộ thẻ card, tiêu đề, input, select và chữ hiển thị tự động co giãn và chuyển đổi màu sáng/tối sắc nét, tương phản hoàn hảo.
