...
2. **Giao dien & Tham my:**
   ...
   - **Responsive UI Layout & Button Scaling (CRITICAL):** TUYỆT ĐỐI KHÔNG ép cố định `width: 100%` cho các nút bấm phụ (như nút "Gửi", "Lọc", "Thao tác") bên trong các container nằm ngang (flex row) khiến nút bị giãn dài quá khổ. Các container chính (`.view`, `#view-user-management`, `#view-chat-support`) phải được thiết kế Responsive tự điều chỉnh độ rộng (`width: 100%; box-sizing: border-box; flex: 1;`), co giãn mượt mà khi người dùng thay đổi kích thước cửa sổ.
    - **Responsive Data Tables & Mobile/Compact Layout (CRITICAL UI/UX):** Đối với tất cả các bảng dữ liệu (Data Tables) có nhiều cột (> 4-5 cột như Quản lý User/Staff, Feedback, Lịch sử thông báo...):
      ...
      2. Khi thu nhỏ cửa sổ ứng dụng (hoặc màn hình hẹp `< 900px`), hệ thống phải tự động chuyển đổi linh hoạt từ dạng Bảng (`<table>`) sang dạng **Danh sách Thẻ Responsive (Card Grid Layout)**. Mỗi Card chứa đầy đủ thông tin (Header, Trạng thái, Thông số, Nút Thao Tác) giúp trải nghiệm UI/UX tự nhiên, mượt mượt trên mọi kích thước màn hình mà không bị méo hay che khuất cột.
    - **Responsive Web & Mobile Layout System (CRITICAL RESPONSIVE SKILL):**
      1. **100% RESPONSIVE TRÊN MỌI THIẾT BỊ (Mobile, Tablet, Desktop):** Mọi trang và component trên `apps/web` (Header, Landing Page Hero, Pricing Grid, Testimonials Marquee, Event Popup, Footer) bắt buộc phải phản ứng linh hoạt theo độ rộng màn hình qua CSS Media Queries (`@media (max-width: 1024px)`, `@media (max-width: 768px)`, `@media (max-width: 480px)`).
      2. **Thanh Navbar & Navigation Mobile:** Trên màn hình di động (`< 768px`), thanh Navigation lơ lửng (`.apple-nav`) phải thu gọn kích thước, ẩn bớt nav items không cần thiết hoặc chuyển sang menu rút gọn mượt mà, không bao giờ làm vỡ khung hay tràn lề màn hình.
      3. **Hero Section & Pricing Grid:** Chuyển đổi linh hoạt các bố cục nhiều cột (Multi-column flex/grid) sang dạng 1 cột stacked tự nhiên trên màn hình nhỏ. Khung chữ, hình ảnh minh họa và nút bấm tự động điều chỉnh font-size và padding phù hợp với ngón tay chạm của người dùng di động.
   ...
   - **No LocalStorage API URL Hijacking & Environment Truth (CRITICAL CONFIG SKILL):**
     1. **TUYỆT ĐỐI KHÔNG DÙNG LOCALSTORAGE ĐỂ ĐÈ LÊN API URL CỦA BACKEND SERVER (No LocalStorage Config Hijacking):** `localStorage` chỉ là bộ nhớ tạm trên trình duyệt của máy khách, KHÔNG PHẢI BACKEND. TUYỆT ĐỐI KHÔNG ưu tiên đọc `localStorage` để quyết định đường dẫn API Server vì sẽ làm mốc URL bị "đóng băng" ở giá trị cũ (`/api/auth/login`), dẫn tới lỗi `Cannot POST /api/auth/login` khi Server đã đổi `API_PREFIX` (ví dụ `/api/eigu-v1-t24v02c03/auth/login`).
     2. Mọi cấu hình URL API / WebSocket phải dựa trên nguồn tri thức thực của Backend (`process.env.API_PREFIX`, IPC từ Main Process, hoặc file cấu hình tập trung [config.js](file:///Users/peggy2402/Projects/eigu-platform/apps/desktop/src/assets/js/config.js)), đảm bảo 100% requests khớp chính xác với `API_PREFIX` do Server phát hành.
   - **Production & Client Readiness First Mindset Skill (CRITICAL PRODUCTION DEPLOYMENT SKILL):**
    ...
    - **Mandatory UI Text Internationalization with `data-i18n` (CRITICAL I18N SKILL):**
      ...
      3. **i18n cho JS logic (toast messages, dynamic strings):** Khi viết JS có chứa chuỗi văn bản hiển thị (ví dụ `showToast('...', '...', 'success')`), BẮT BUỘC phải dùng hàm `t('key')` (định nghĩa trong `settings.js`). Các key cho toast nên đặt tiền tố `toast_*`. Key name convention: `snake_case` theo chức năng (VD: `toast_login_success_title`, `chat_ai_greeting`).
      4. **Cấu trúc I18N_DICTIONARY:** File `settings.js` chứa object `I18N_DICTIONARY` với 2 section: `vi` (Tiếng Việt) và `en` (English). Mỗi section là key-value map. Khi thêm một key mới LUÔN LUÔN phải thêm cả bản dịch VI và EN. Các chuỗi ký tự Việt Nam trong VI dict phải dùng Unicode escape sequences (VD: `\u0110` thay vì `Đ`) để tránh lỗi encoding.
      5. **Hàm `t(key)` hỗ trợ JS runtime translation:** Hàm `t(key)` (định nghĩa trong `settings.js`) đọc ngôn ngữ từ `localStorage.getItem('eigu_language')` (mặc định `'vi'`) và lookup trong `I18N_DICTIONARY`. Dùng `t('key')` TRONG MỌI chuỗi JS được render ra giao diện. KHÔNG hardcode tiếng Việt trong JS khi nó hiển thị ra UI.
      6. **`data-i18n-placeholder` cho input:** Với `<input>` và `<textarea>`, dùng `data-i18n-placeholder="key"` thay vì `data-i18n` (vì `applyAppLanguage` xử lý riêng placeholder). Với contenteditable div, dùng `data-i18n-placeholder="key"` kết hợp `data-placeholder="..."` làm fallback.
    - **Strict Notification Targeting & User Data Isolation Skill (CRITICAL PRIVACY SKILL):**
        1. **TUYỆT ĐỐI BẢO MẬT DỮ LIỆU RIÊNG TƯ THÔNG BÁO THEO TÀI KHOẢN (No Cross-User Notification Leakage):** Thông báo tài chính (Nạp tiền, trừ tiền, biến động số dư, đổi mật khẩu, cấp lại OTP) BẮT BUỘC phải cài đặt `target = <userId>` hoặc `target = user:<userId>` đích danh người nhận. TUYỆT ĐỐI KHÔNG để `target = 'user'` hay `target = 'all'` khiến toàn bộ người dùng khác trong hệ thống nhìn thấy biến động số dư của nhau.
        2. **Lọc Thông Báo Cá Nhân Hóa 4 Cấp Độ:** Phân hệ Backend (`NotificationsService.findAllForUser`) BẮT BUỘC đọc JWT của người dùng hiện tại và chỉ trả về các thông báo thỏa mãn 1 trong 4 điều kiện: (1) `target = 'all'`, (2) `target = role`, (3) `target = userId` / `user:userId`, (4) `target = email` / `email:userEmail`.
    - **Modern Dark Custom Form Controls & Select Dropdowns (CRITICAL UI/UX SKILL):**
       1. **TUYỆT ĐỐI KHÔNG ĐỂ THẺ `<select>` XUẤT HIỆN DẠNG KHUNG TRẮNG NATIVE MẶC ĐỊNH BÌNH THƯỜNG:** Mọi thẻ chọn `<select>` trên Desktop/Web phải được bọc CSS Dark Mode hoàn chỉnh (`background: var(--bg-card)`, `color: var(--text-primary)`, `border: 1px solid var(--border-color)`, `appearance: none`, custom arrow SVG icon), đồng bộ tuyệt đối với theme tối của EIGU Platform.
       2. Mọi thẻ `<option>` bên trong select phải sử dụng biến CSS (`background: var(--bg-card)`, `color: var(--text-primary)`) để tự động tương thích với cả giao diện Tối và Sáng.
       3. **CHUYỂN ĐỔI THEME TỰ ĐỘNG KHÔNG HARDCODE MÀU NỀN TỐI HỘP CỨNG (CRITICAL THEME RULE):** TUYỆT ĐỐI KHÔNG hardcode màu nền tối cố định kiểu `background: rgba(15, 23, 42, 0.8)`, `rgba(20, 20, 25, 0.96)` hoặc `#0f172a` cho bất kỳ container, card, mobile menu drawer (`.mobile-nav-drawer`), header hay popup modal nào trong CSS/HTML/JS inline. BẮT BUỘC sử dụng biến CSS theme (`var(--bg-card)`, `var(--bg-primary)`, `var(--bg-secondary)`, `var(--text-primary)`, `var(--border-color)`) để khi người dùng chuyển giữa **Giao diện Tối (Dark)** và **Giao diện Sáng (Light)**, toàn bộ giao diện (kể cả menu mobile, drawer, backdrop, tiêu đề và các nút điều hướng) tự động chuyển đổi màu sáng/tối đồng bộ, không bao giờ bị hiện tượng chữ tối trên nền tối hay vỡ giao diện ở chế độ sáng.
       4. **TUYỆT ĐỐI KHÔNG ĐỂ NÚT CHỌN TỆP (`<input type="file">`) XUẤT HIỆN DẠNG NÚT TRẮNG THÔ NATIVE MẶC ĐỊNH:** Mọi nút bấm tải tệp (`input[type="file"]::file-selector-button` và `::-webkit-file-upload-button`) phải được custom CSS bo góc mượt mà (`border-radius: 8px`, `background: var(--bg-card)`, `color: var(--text-primary)`, `border: 1px solid var(--border-color)`, hiệu ứng hover co giãn), tự động tương thích với cả giao diện Tối và Sáng.
       5. **MÀU CHỮ VÀ HIỆU ỨNG HOVER TƯƠNG THÍCH ĐA THEME (CRITICAL HOVER COLOR RULE):** TUYỆT ĐỐI KHÔNG hardcode màu chữ trắng `color: '#ffffff'` hoặc `#fff` khi rê chuột (hover) vào các thẻ card, container hay tiêu đề trên phông nền `var(--bg-card)` hoặc `var(--bg-primary)`. Vì khi ở Chế độ Sáng (Light Mode), phông nền card là màu trắng `#ffffff`, việc đổi màu chữ thành `#ffffff` khi hover sẽ khiến tiêu đề bị trùng màu trắng hoàn toàn với background và biến mất (White text on White background). BẮT BUỘC sử dụng màu accent của card (`item.accentColor`) hoặc giữ biến CSS theme (`var(--text-primary)`, `var(--accent)`) khi hover để tiêu đề hiển thị nổi bật, rõ nét trên cả Giao diện Tối và Giao diện Sáng.
